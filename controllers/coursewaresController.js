...
exports.createCourseware = async (req, res) => {
    try {
        ...
        const newCourseware = new Courseware({ title, text, quiz, createdBy: req.user.id });
        await newCourseware.save();
        res.status(201).json({ message: "Courseware created successfully", courseware: newCourseware });
    } catch (err) {
...
exports.getCoursewareById = async (req, res) => {
    try {
        ...
        if (!courseware) return res.status(404).json({ message: 'Courseware not found' });
        res.json(courseware);
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
...
exports.updateCourseware = async (req, res) => {
    try {
        const courseware = await Courseware.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!courseware) return res.status(404).json({ message: 'Courseware not found' });
        res.json(courseware);
    } catch (err) {
        res.status(400).json({ message: err.message });
